...
export default function SettingsTab() {
  ...
  useEffect(() => {
    setGiftRangesDraft((settings.giftRanges || []).filter((range) => !isLegacyFreeShippingGiftRange(range)));
  }, [settings.giftRanges]);
  ...
  const addGiftRange = () => {
    ...
    const initialProduct = eligibleGiftProducts[0];
    if (!initialProduct) {
      showToast('Aucun produit publié et en stock ne peut être offert.', 'error');
      return;
    }
    setGiftRangesDraft((current) => [
      ...current,
      {
        ...
        productId: Number(initialProduct.id),
        productName: getGiftProductName(Number(initialProduct.id)),
        isActive: true,
      },
    ]);
  };
          ...
          <form onSubmit={onSaveGeneralSettings} className={`border rounded-3xl p-6 space-y-6 transition-all duration-200 ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                { label: 'Livraison Gratuite à partir de (DH)', name: 'freeShippingThreshold', defaultValue: FREE_SHIPPING_SUBTOTAL_DH, type: 'number', align: 'text-right', readOnly: true },
                ...
                <div key={idx} className="space-y-1.5">
                  ...
                  <input
                    ...
                    required
                    readOnly={'readOnly' in inp && Boolean(inp.readOnly)}
                  />
                </div>
              ))}
            </div>
                              ...
                              <div className={`border rounded-2xl p-4 mt-2 space-y-4 ${
                                ...
                                <div className="flex justify-end gap-2 pt-2 border-t border-slate-200/50 dark:border-slate-800/40">
                                  ...
                                  <button
                                    ...
                                    className="po-ui-button po-ui-button--primary po-ui-button--md px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl text-[10px] uppercase font-black"
                                  >
                          ...
                          <div className="space-y-4">
                            <div className={`border-b pb-3 flex justify-between items-center ${adminTheme === 'light' ? 'border-slate-100' : 'border-slate-800'}`}>
                              ...
                              <button
                                ...
                                className="po-ui-button po-ui-button--primary po-ui-button--md px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-[10px] uppercase tracking-wider rounded-xl transition cursor-pointer"
                              >
                            ...
                            {isAddingFaq && (
                              <form onSubmit={onAddFaqSubmit} className={`border rounded-2xl p-4 space-y-4 ${
                                ...
                                <div className="flex justify-end gap-2 pt-2 border-t border-slate-200/50 dark:border-slate-800/40">
                                  <button
                                    type="submit"
                                    className="po-ui-button po-ui-button--primary po-ui-button--md px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl text-[10px] uppercase font-black"
                                  >
                                    Ajouter la Question
                                  </button>
                                </div>
                            ...
                            <div className="space-y-4">
                              <div className="flex justify-between items-center border-b pb-2">
                                ...
                                <button
                                  ...
                                  className="po-ui-button po-ui-button--primary po-ui-button--md px-2.5 py-1 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-[10px] uppercase tracking-wider rounded-lg transition"
                                >
                                  Ajouter
                                </button>
                              </div>
                            ...
                            {openGiftPickerIndex === index && (
                              <div className={`relative mt-2 w-full overflow-hidden rounded-xl border shadow-lg ${adminTheme === 'light' ? 'border-slate-200 bg-white shadow-slate-900/10' : 'border-slate-700 bg-slate-950 shadow-black/40'}`}>
                                <div className={`border-b p-2.5 ${adminTheme === 'light' ? 'border-slate-100 bg-slate-50/80' : 'border-slate-800 bg-slate-900/40'}`}>
                                  ...
                                  <p className="mt-2 text-[10px] font-semibold normal-case text-slate-500">Choisissez un produit publié et en stock.</p>
                                </div>
                                <div role="listbox" aria-label="Cadeaux disponibles" className="max-h-72 overflow-y-auto p-1.5">
                                  {matchingProducts.length > 0 ? matchingProducts.map((product: any) => {
                                    ...
                                    return <button
                                      ...
                                      key={product.id}
                                      onClick={() => {
                                        updateGiftRange(index, { productId: Number(product.id), productName: getGiftProductName(Number(product.id)) });
                                        setGiftProductQueries((current) => ({ ...current, [index]: '' }));
                                    ...
                                    >
                                      ...
                                      <span className="min-w-0 flex-1 normal-case">
                                        ...
                                        <span className="mt-0.5 block truncate text-[10px] font-medium text-slate-500">
                                          {`${product.brand || product.vendor || 'Catalogue'}${product.sku ? ` · SKU ${product.sku}` : ''}`}
                                        </span>
                                      </span>
                                      <span className="shrink-0 rounded-md bg-emerald-50 px-2 py-1 text-[10px] font-black normal-case text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
                                        {`${getGiftProductStock(product)} en stock`}
                                      </span>
          ...
          <div className={`border rounded-3xl p-6 space-y-6 transition-all duration-200 ${
            ...
            <div className={`border-b pb-3 flex justify-between items-center ${adminTheme === 'light' ? 'border-slate-100' : 'border-slate-900'}`}>
              ...
              <button
                ...
                className="po-ui-button po-ui-button--primary po-ui-button--md px-3.5 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-black text-[10px] uppercase tracking-wider rounded-xl hover:from-emerald-400 hover:to-teal-400 transition"
              >
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-xl text-[9px] font-extrabold uppercase tracking-wider transition cursor-pointer ${
                              op.isActive
                                ...
                                : 'po-ui-button po-ui-button--primary po-ui-button--md bg-emerald-500 text-slate-950 hover:bg-emerald-400 font-black shadow-md shadow-emerald-500/20'
                            }`}
                          >
              ...
              <div className={`flex items-center justify-between p-4 rounded-2xl border ${
                ...
                {isMfaEnabled ? (
                  ...
                  <button
                    ...
                    className="po-ui-button po-ui-button--primary po-ui-button--md px-3 py-1.5 text-[10px] font-black uppercase tracking-wider bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 rounded-xl hover:opacity-90 transition shrink-0"
                  >
                    Activer le MFA
                  </button>
                )}
                ...
                <div className={`p-5 rounded-2xl border space-y-4 ${
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="po-ui-button po-ui-button--primary po-ui-button--md px-4 py-2 bg-emerald-500 text-slate-950 font-bold text-xs rounded-xl hover:bg-emerald-400 transition disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
                    >
                      Confirmer
                    </button>
                  </div>
            ...
            <div className={`border rounded-3xl p-6 space-y-5 transition-all duration-200 ${
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="po-ui-button po-ui-button--primary po-ui-button--md px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-bold text-xs uppercase tracking-wider rounded-xl hover:opacity-90 transition"
                >
