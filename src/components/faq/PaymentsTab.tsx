    ...
    <div className="flex-1 flex flex-col justify-between gap-6 animate-[fadeIn_0.4s_ease-out]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        ...
        <div className="flex flex-col gap-3">
          ...
          {([
            { id: 'card', name: text.payCard, icon: CreditCard },
            { id: 'cod',  name: text.payCod,  icon: MapPin },
          ] as const).map((m) => {
            const isMethodActive = payMethod === m.id;
            const Icon = m.icon;
            return (
              <button
                key={m.id}
                ...
                <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <span
                    className={`text-xs font-black tracking-tight ${
                      isMethodActive ? 'text-indigo-700' : 'text-slate-700'
                    }`}
                  >
                    {m.name}
                  </span>
          ...
          {payMethod === 'card' ? (
            ...
            >
              ...
              <div className="flex flex-col gap-1 mt-4">
                <span className="text-[12px] font-mono tracking-[0.25em] text-slate-300">
                  **** **** **** 8820
                </span>
              ...
              <div className="flex items-center justify-between border-t border-slate-800/80 pt-2.5 mt-2">
                <span className="text-[7.5px] font-black uppercase text-indigo-400 tracking-wider">
                  {text.paySecureBadge}
                </span>
                <div className="flex gap-2">
                  <span className="px-1.5 py-0.5 bg-white/10 rounded border border-white/5 font-sans text-[7px] font-bold text-slate-300">
                    CMI
                  </span>
                  <span className="px-1.5 py-0.5 bg-white/10 rounded border border-white/5 font-sans text-[7px] font-bold text-slate-300">
                    VISA
            ...
            <div className="w-72 h-44 bg-slate-50 border border-slate-200/50 rounded-2xl p-5 flex flex-col justify-between items-center text-center shadow-inner">
              ...
              <p className="text-[9.5px] font-semibold text-slate-400 leading-normal max-w-[180px] mb-2">
                {text.payCodDesc}
              </p>
            </div>
          )}
